...
def trainer(config, run_name=config['wandb']['run_name'], no_sweep = True):
    ...
    train_dataset = Paddy(images_dirs=[config['data']['train_dir']],
    ...
    valid_dataset = Paddy(images_dirs=[config['data']['val_dir']],
                            img_preprocessor=img_preprocessor,
                            of_num_imgs=None,
                            overfit_test=False,
                            augment_data=False)
    ...
    for epoch in range(epochs):
        ...
        valid_metrics = ["accuracy", "f1_score",'precision','recall','confusion_matrix']
        ...
        PlotHeatMap(epoch,valid_gt,valid_pred, path = config['model']['heatmap_path'])
        ...
        if wandb_log:
            ...
            wandb.log({'acc/train': train_scores['accuracy'],
                        ...
                        'precision/val': valid_scores['precision'],
                        'recall/val': valid_scores['recall'],
                        'confusion_matrix/val': valid_scores['confusion_matrix']
            }, step=epoch)
